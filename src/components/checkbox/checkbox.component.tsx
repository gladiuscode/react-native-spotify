...
function WithLeftText({style, size, text, initialValue = false}: WithLeftText) {
  return (
    <Container style={style} initialValue={initialValue}>
...
function WithRightText({style, size, text, initialValue = false}: WithRightText) {
  return (
    <Container style={style} initialValue={initialValue}>
      <Check size={size} />
...
function Container({style, initialValue = false, children}: ContainerProps) {
  ...
  const handleOnPress = useCallback(() => setChecked((current) => !current), []);

  const handleOnPressIn = useCallback(() => setPressed((current) => !current), []);

  const handleOnPressOut = useCallback(() => setPressed((current) => !current), []);
  ...
  return (
    <Provider value={{
      ...
      <Pressable style={[style, styles.container]} onPress={handleOnPress} onPressIn={handleOnPressIn} onPressOut={handleOnPressOut}>
        {children}
      </Pressable>
    </Provider>
  )
...
function Check({ size }: CheckProps) {
  ...
  const dimensions = size ? {width: size, height: size, borderRadius: size * 2} : undefined;

  return (
    <View style={[styles.checkbox, checked ? styles.checkedCheckbox : undefined, pressed ? styles.pressedCheckbox : undefined, dimensions]}>
      {checked ? <Feather name="check" size={styles.checkbox.width - 8} color={appTheme.colors.black}/> : null}
    </View>
  );
}
